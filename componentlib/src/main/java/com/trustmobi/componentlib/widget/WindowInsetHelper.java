package com.trustmobi.componentlib.widget;
...
import com.trustmobi.basiclib.utils.DisplayHelper;
...
public class WindowInsetHelper {
    ...
    public WindowInsetHelper(ViewGroup viewGroup, IWindowInsetLayout windowInsetLayout) {
        mWindowInsetLayoutWR = new WeakReference<>(windowInsetLayout);
        KEYBOARD_HEIGHT_BOUNDARY = DisplayHelper.dp2px(viewGroup.getContext(), 100);
    ...
    public boolean defaultApplySystemWindowInsets19(ViewGroup viewGroup, Rect insets) {
        ...
        if (insets.bottom >= KEYBOARD_HEIGHT_BOUNDARY) {
            ViewHelper.setPaddingBottom(viewGroup, insets.bottom);
            insets.bottom = 0;
        } else {
            ViewHelper.setPaddingBottom(viewGroup, 0);
        }
    ...
    public boolean defaultApplySystemWindowInsets21(ViewGroup viewGroup, WindowInsetsCompat insets) {
        ...
        if (insets.getSystemWindowInsetBottom() >= KEYBOARD_HEIGHT_BOUNDARY) {
            showKeyboard = true;
            ViewHelper.setPaddingBottom(viewGroup, insets.getSystemWindowInsetBottom());
        } else {
            ViewHelper.setPaddingBottom(viewGroup, 0);
        }
